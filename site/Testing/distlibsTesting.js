...
function StdTubeRating(tLiq, length, style, refrgt, outletSize,tSuct) {
    ...
    if (style == "nozzle") {
        ...
        if (refrgt == "R-22" || refrgt == "R-12" ||
            ...
            qStd = 86.863415283 * Math.pow(eval(outletSize), 3.369915399);
...
//------------------ Testing -----------------------------------------
 ...
 RateVenturi();
console.log("Q = " + q);
var refList = ["R-22", "R-12", "R-134A", "R-401A", "R-407C", "R-410A", "R-502", "R-402A", "R-404A", "R-507"];


var csvArr = [["Refrigerant", "Circuit Size", "Temp", "Capacity"]];

function TestPrintEntries(csvArr) {
    var stdTubeRatingValue = StdTubeRating(tLiq, length, style, refrgt, outletSize, tSuct);
...
function TestAllStdTubeRating(csvArr) {
    var tSuctArray = [40, 20, 0, -20, -40];
    var outletSizeArray = ["3/16", "1/4", "5/16", "3/8"];
    for (var i = 0; i < refList.length; i++) {
        refrgt = refList[i];
        for (let i = 0; i < outletSizeArray.length; i++) {
            outletSize = outletSizeArray[i];
            for (let _a = 0; _a < tSuctArray.length; _a++) {
                tSuct = tSuctArray[_a];
                csvArr = TestPrintEntries(csvArr);
            }
        }
        csvArr.push([" ", " ", " ", " "]);
    }
